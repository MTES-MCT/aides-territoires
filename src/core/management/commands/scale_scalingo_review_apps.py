...
class Command(BaseCommand):
    ...
    def get_scalingo_bearer_token(self):
        ...
        res = requests.post(endpoint, headers=API_HEADERS, auth=('', settings.SCALINGO_API_TOKEN))  # noqa
        data = res.json()
        return data['token']
    ...
    def scale_review_apps(self, app_names, direction=SCALING_DIRECTION_DEFAULT):  # noqa
        ...
        for app_name in app_names:
            ...
            res = requests.post(endpoint, headers=API_HEADERS, data=json.dumps(data))  # noqa
            data = res.json()
