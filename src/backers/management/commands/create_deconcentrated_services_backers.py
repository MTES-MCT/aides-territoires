...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        confirm = input(
            "This command is meant to run in production only "
            "once.\nAre you sure you know what you're doing?\n"
        )
        if confirm != "yes":
            return

        regions = Perimeter.objects.filter(scale=Perimeter.SCALES.region).exclude(
            code__in=OVERSEAS_REGIONS
        )

        backers = []
        for region in regions:
            for service in SERVICES:
                backers.append(Backer(name="{} — {}".format(service, region.name)))

        results = Backer.objects.bulk_create(backers)
        self.stdout.write(self.style.SUCCESS("{} backers created".format(len(results))))
