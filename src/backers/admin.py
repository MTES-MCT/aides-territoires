...
class BackerAdmin(ImportExportActionModelAdmin):
    ...
    fieldsets = [
        ...
        ("Groupe de porteurs", {"fields": ("group",)}),
        (
            "Données diverses",
            {
                "fields": (
    ...
    def display_related_aids(self, obj):
        ...
        for aid in obj.financed_aids.all().order_by("name").select_related("author"):
    ...
    def display_related_themes(self, obj):
        ...
        categories = (
            Category.objects.select_related("theme")
        ...
        for theme, theme_categories in themes:
            related_themes_html += format_html(
                "<li><strong>{theme} > </strong>", theme=theme
            )
            for k, theme_category in theme_categories:
                related_themes_html += format_html(
                    "{category}. ", category=theme_category
                )
    ...
    def display_related_programs(self, obj):
        ...
        programs = Program.objects.filter(aids__in=obj.financed_aids.all()).distinct()

        related_programs_html = format_html("<ul>")
        for program in programs:
            related_programs_html += format_html(
                "<li><strong>{program_name}</strong></li>", program_name=program.name
            )
        related_programs_html += format_html("</ul>")
