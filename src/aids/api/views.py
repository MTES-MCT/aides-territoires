...
from aids.api.serializers import (
    ...
    AidTypeSerializer,
    AidStepSerializer,
    AidRecurrenceSerializer,
    AidDestinationSerializer,
)
...
class AidViewSet(
    ...
    def get_base_queryset(self):
        ...
        qs = (
            Aid.objects.select_related("perimeter")
            .prefetch_related(
    ...
    @extend_schema(
        summary="Afficher l'aide donnée", tags=[Aid._meta.verbose_name_plural]
    )
    @cache_detail_page
...
class AidAudiencesViewSet(mixins.ListModelMixin, viewsets.GenericViewSet):
    ...
    def get_queryset(self):
        ...
        for (audience_type, audience_group) in AUDIENCES_GROUPED:
            aid_audiences += [
                {"id": id, "name": name, "type": audience_type}
                for (id, name) in audience_group
            ]  # noqa
