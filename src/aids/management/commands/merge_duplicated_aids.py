        ...
        for old_aid in old_aids:
            if Aid.objects.filter(
                ...
                try:
                    ...
                    old_aid.recoverable_advance_amount = (
                        duplicated_aid.recoverable_advance_amount
                    )
                    ...
                    old_aid.import_raw_object_calendar = (
                        duplicated_aid.import_raw_object_calendar
                    )
                    old_aid.import_raw_object_temp = (
                        duplicated_aid.import_raw_object_temp
                    )
                    old_aid.import_raw_object_temp_calendar = (
                        duplicated_aid.import_raw_object_temp_calendar
                    )
                    old_aid.search_vector_unaccented = (
                        duplicated_aid.search_vector_unaccented
                    )
                    ...
                    if duplicated_aid.projects is not None:
                        for project in duplicated_aid.projects.all():
                            aidproject = AidProject.objects.get(
                                aid=duplicated_aid.pk, project=project.pk
                            )
                        ...
                        for (
                            ...
                            suggested_aidproject = SuggestedAidProject.objects.get(
                                aid=duplicated_aid.pk, project=suggested_project.pk
                            )
                    ...
                    merged_aids += 1
                    logger.info(
                        f"duplicated aid {duplicated_aid.pk} merged in old aid {old_aid.pk}"
                    )
