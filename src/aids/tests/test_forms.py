...
def test_search_from_filter_by_audiances(aids):
    ...
    form = AidSearchForm({
        'targeted_audiances': ['commune', 'department']
    })
    qs = form.filter_queryset(aids)
    assert qs.count() == 2
    for aid in qs:
    ...
    form = AidSearchForm({
        'targeted_audiances': ['commune', 'department', 'region']
    })
    qs = form.filter_queryset(aids)
    assert qs.count() == 3
    for aid in qs:
    ...
    form = AidSearchForm({
        'targeted_audiances': ['commune', 'department', 'region', 'epci']
    })
    qs = form.filter_queryset(aids)
    assert qs.count() == 4
    for aid in qs:
    ...
    form = AidSearchForm({
        'targeted_audiances': ['commune', 'department',
                               'region', 'epci', 'lessor', 'association']
    })
    qs = form.filter_queryset(aids)
    assert qs.count() == 6
    for aid in qs:
