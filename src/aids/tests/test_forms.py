...
def test_search_from_filter_by_audiences(aids):
    ...
    form = AidSearchForm({"targeted_audiences": ["commune", "department", "region"]})
    qs = form.filter_queryset(aids)
    assert qs.count() == 3
    for aid in qs:
        assert any(
...
def test_aid_calendar_fields_validation(aid_form_data):
    ...
    aid_form_data.update({"recurrence": "oneoff", "submission_deadline": ""})
    form = AidEditForm(aid_form_data, requested_status="reviewable")
    assert not form.is_valid()

    aid_form_data.update({"recurrence": "oneoff", "submission_deadline": "01/01/2020"})
    form = AidEditForm(aid_form_data, requested_status="reviewable")
    assert form.is_valid()

    aid_form_data.update({"recurrence": "recurring", "submission_deadline": ""})
    form = AidEditForm(aid_form_data, requested_status="reviewable")
    assert not form.is_valid()

    aid_form_data.update(
        {"recurrence": "recurring", "submission_deadline": "01/01/2020"}
    )
    form = AidEditForm(aid_form_data, requested_status="reviewable")
    assert form.is_valid()

    aid_form_data.update({"recurrence": "ongoing", "submission_deadline": ""})
    form = AidEditForm(aid_form_data, requested_status="reviewable")
    assert form.is_valid()
