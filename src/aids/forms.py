...
class AidTypesWidget(forms.MultiWidget):
    ...
    def decompress(self, value):
        if value:
        # If value contains more than 2 values, we have to regroup value[1:]
        # because multiwidget only accepts a number of values equal to the number
        # of subwidgets.
            if value[1]:
                technical_values = value[1].split()
                technical_values.extend(value[2:])
                value = [value[0], technical_values]
                return value
            return value
        return ['', '']
