...
def update_search_vector_unaccented(apps, schema_editor):
    ...
    for aid in aids:
        ...
        search_vector_unaccented = \
            ...
            SearchVector(Value(' '.join(backer.name for backer in backers), output_field=models.CharField()), weight='D', config='french_unaccent') + \
            SearchVector(Value(' '.join(category.name for category in categories), output_field=models.CharField()), weight='D', config='french_unaccent')
