...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        allowed = (
            settings.ENV_NAME == "production" and settings.EXPORT_CONTACTS_ENABLED
        )  # noqa
        if not allowed:
    ...
    def export_account(self, user):
        ...
        if latest_draft:
            ...
            attributes.update({"DATE_DERNIER_BROUILLON": draft_date})
        if latest_expired:
            expiration_date = latest_expired.submission_deadline.isoformat()
            attributes.update({"DATE_DERNIERE_EXPIRATION": expiration_date})
        data = {
            "email": user.email,
