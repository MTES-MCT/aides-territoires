...
@app.task
def export_eligibility_tests_stats_as_csv(
    eligibility_tests_id_list, author_id, background=True
):  # noqa
    """
    ...
    eligibility_test = EligibilityTest.objects.get(
        id=eligibility_tests_id_list[0]
    )  # noqa
    eligibility_test_questions = (
        eligibility_test.eligibilitytestquestion_set.select_related("question").all()
    )
    eligibility_test_stats = AidEligibilityTestEvent.objects.select_related(
        "aid"
    ...
    csv_writer = csv.writer(
        csv_buffer if background else response, delimiter=",", dialect="excel"
    )  # noqa
    ...
    for eligibility_test_event in eligibility_test_stats:
        eligibility_test_event_row = [
            eligibility_test.name,
            eligibility_test_event.aid,
        ]  # noqa
        ...
        for question_id in question_id_list:
            answer = next(
                (
                    a
                    for a in eligibility_test_event.answer_details
                    if a["id"] == question_id
                ),
                None,
            )  # noqa
