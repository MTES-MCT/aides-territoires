...
class NouvelleAquitaineSpider(scrapy.Spider):
    ...
    start_urls = ["https://les-aides.nouvelle-aquitaine.fr/fiches-rss.xml"]
    ...
    def aid_parse(self, response):
        ...
        categorie = response.css(
            "ul.m-breadcrumb__list > li:nth-child(2) > span::text"
        ).get()

        is_call_for_project = False
        if (
            response.css("span.tag-appel-projet").get()
            or response.css("span.tag-appel-manifestation").get()
        ):
        ...
        for index, item in enumerate(response.css("div.categories")):
            ...
            aid_header[aid_header_key] = (
                content_prettify(item.css("p::text").get())
                .strip()
                .replace("\n", "")
                .replace("   ", "")
                .replace("  ,  ", ";")
        ...
        yield {
            ...
            + "<br />"
            + content_prettify(aid_details["objectifs"]),
            "categorie": categorie,
