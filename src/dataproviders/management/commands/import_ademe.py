...
DATA_SOURCE = DataSource.objects.prefetch_related("perimeter", "backer").get(pk=5)

AUDIENCES_MAPPING_CSV_PATH = (
    os.path.dirname(os.path.realpath(__file__))
    + "/../../data/ademe_audiences_mapping.csv"
)
...
AUDIENCES_DICT = mapping_audiences(
    AUDIENCES_MAPPING_CSV_PATH, SOURCE_COLUMN_NAME, AT_COLUMN_NAMES
)
...
class Command(BaseImportCommand):
    ...
    def handle(self, *args, **options):
        ...
        regions_qs = Perimeter.objects.filter(scale=Perimeter.SCALES.region)
    ...
    def extract_import_raw_object_calendar(self, line):
        ...
        import_raw_object_calendar["date_ouverture"] = line.find(
            ".//date_ouverture"
        ).text
        import_raw_object_calendar["annee_publication"] = line.find(
            ".//annee_publication"
        ).text
        import_raw_object_calendar["date_publication"] = line.find(
            ".//date_publication"
        ).text
    ...
    def extract_start_date(self, line):
        ...
        publication_date = datetime.strptime(publication_date_text, "%d/%m/%Y %H:%M:%S")
        return publication_date

    def extract_submission_deadline(self, line):
        closure_date_text = line.find(".//date_cloture").text
        closure_date = datetime.strptime(closure_date_text, "%d/%m/%Y").date()
        return closure_date
