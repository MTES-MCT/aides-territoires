...
DATA_SOURCE = DataSource.objects.prefetch_related("perimeter", "backer").get(pk=4)
...
AUDIENCES_MAPPING_CSV_PATH = (
    ...
    + "/../../data/ile_de_france_audiences_mapping.csv"
)
...
CATEGORIES_MAPPING_CSV_PATH = (
    os.path.dirname(os.path.realpath(__file__))
    + "/../../data/ile_de_france_categories_mapping.csv"
)
    ...
    def extract_targeted_audiences(self, line):
        ...
        for audience in audiences:
            if audience["title"] in AUDIENCES_DICT:
                ...
                self.stdout.write(
                    self.style.ERROR(f"Audience '{audience_title}' not mapped")
                )
        ...
        if categories != [""]:
            for category in categories:
                if category["title"] in CATEGORIES_DICT:
                    ...
                    self.stdout.write(
                        self.style.ERROR(f"Category '{category_title}' not mapped")
                    )
        else:
        ...
        if line.get("dateOuvertureCampagne", None):
            try:
                start_date = datetime.strptime(
                    line["dateOuvertureCampagne"], "%Y-%m-%dT%H:%M:%S.%fZ"
                )
            except Exception:
                start_date = datetime.strptime(
                    line["dateOuvertureCampagne"], "%Y-%m-%dT%H:%M:%SZ"
                )
            return start_date
        ...
        if line.get("dateFinCampagne", None):
            try:
                submission_deadline = datetime.strptime(
                    line["dateFinCampagne"], "%Y-%m-%dT%H:%M:%S.%fZ"
                )
            except Exception:
                submission_deadline = datetime.strptime(
                    line["dateFinCampagne"], "%Y-%m-%dT%H:%M:%SZ"
                )
