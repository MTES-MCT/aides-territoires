...
DATA_SOURCE = DataSource.objects.prefetch_related("perimeter", "backer").get(pk=6)
...
AUDIENCES_MAPPING_CSV_PATH = (
    ...
    + "/../../data/plan_france_relance_audiences_mapping.csv"
)
...
CATEGORIES_MAPPING_CSV_PATH = (
    os.path.dirname(os.path.realpath(__file__))
    + "/../../data/plan_france_relance_categories_mapping.csv"
)
...
class Command(BaseImportCommand):
    ...
    def extract_description(self, line):
        ...
        if "Qui peut en bénéficier ?" in line.get("field_paragraphes", ""):
            desc_2 = str(
                line.get("field_paragraphes", "").partition("Qui peut en bénéficier ?")[
                    0
                ]
            )
    ...
    def extract_contact(self, line):
        if "Liens utiles et contacts" in line.get("field_paragraphes", ""):
            contact = str(
                line.get("field_paragraphes", "").partition("Liens utiles et contacts")[
                    2
                ]
            )
        elif "Documents utiles" in line.get("field_paragraphes", ""):
            contact = str(
                line.get("field_paragraphes", "").partition("Documents utiles")[2]
            )
