...
class BaseImportCommand(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        with transaction.atomic():
            for (
                ...
                try:
                    ...
                    try:
                        import_raw_object_temp = Aid.objects.values_list(
                            "import_raw_object_temp", flat=True
                        ).get(import_uniqueid=aid.import_uniqueid)
                        import_raw_object = Aid.objects.values_list(
                            "import_raw_object", flat=True
                        ).get(import_uniqueid=aid.import_uniqueid)
                        import_raw_object_calendar = Aid.objects.values_list(
                            "import_raw_object_calendar", flat=True
                        ).get(import_uniqueid=aid.import_uniqueid)
                        import_raw_object_temp_calendar = Aid.objects.values_list(
        ...
        success_message = (
            "{} aides total, {} aides crées, {} aides maj, {} aides maj auto".format(
                len(aids_and_related_objects),
                created_counter,
                updated_counter,
                automatic_updated_counter,
            )
        )  # noqa
...
class CrawlerImportCommand(BaseImportCommand):
    ...
    def fetch_data(self, **options):
        ...
        for p in process.crawlers:
            p.signals.connect(add_to_results, signal=scrapy.signals.item_scraped)
        process.start()
