...
class Command(BaseImportCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.perimeter_loire_bretagne = Perimeter.objects.get(
            scale=Perimeter.SCALES.basin, code=LOIRE_BRETAGNE_PERIMETER_CODE
        )
        self.backer_loire_bretagne = Backer.objects.get(id=LOIRE_BRETAGNE_FINANCER_ID)
    ...
    def extract_start_date(self, line):
        try:
            start_date = datetime.strptime(line["Date d'ouverture"], "%m/%d/%Y").date()
        except ValueError:
            start_date = None
        return start_date
