...
DATA_SOURCE = DataSource.objects.prefetch_related("perimeter", "backer").get(pk=1)

AUDIENCES_DICT = {}
AUDIENCES_MAPPING_CSV_PATH = (
    os.path.dirname(os.path.realpath(__file__))
    + "/../../data/nouvelle_aquitaine_rss_audiences_mapping.csv"
)
...
CATEGORIES_MAPPING_CSV_PATH = (
    os.path.dirname(os.path.realpath(__file__))
    + "/../../data/nouvelle_aquitaine_rss_categories_mapping.csv"
)
...
class Command(CrawlerImportCommand):
    ...
    def extract_description(self, line):
        ...
        if line["objectifs"]:
            description += "<strong>{}</strong>: {}<br /><br />".format(
    ...
    def extract_eligibility(self, line):
        ...
        for elem in metadata:
            if line[elem]:
                eligibility += "<strong>{}</strong>: {}<br /><br />".format(
    ...
    def extract_submission_deadline(self, line):
        if line["date_de_fin_de_publication"]:
            ...
            submission_deadline = datetime.strptime(
                line["date_de_fin_de_publication"], submission_deadline_format
            )
