...
from core.forms.fields import ( 
    RichTextField,  AutocompleteModelChoiceField, 
...
class MoreCriteriasWidget(forms.widgets.ChoiceWidget):
    """Custom widget to select project."""

    allow_multiple_selected = True
    template_name = 'search/forms/widgets/more_criterias_widget.html'


class MoreCriteriasSearchForm(forms.Form):
    targeted_audiences = forms.MultipleChoiceField(
        choices=AUDIENCES,
        widget=forms.widgets.MultipleHiddenInput)
    perimeter = forms.CharField(
